...
class DB {
  ...
  constructor() {
    ...
    this.searches = new Map<string, Search>();
  }
  ...
  getHotel = (hotelID: number | string) => {
    const [, hotel] =
  ...
  getHotels = (): Record<string, any> => {
    return this.hotels;
  };

  getHotelsByCountryID = (countryID: string): Record<string, any> => {
...
class Search {
  ...
  constructor(token: string, params: Record<string, any> = {}) {
    this._token = token;
    this._params = params;
    this.readyTimestamp = this._setReadyTimestamp();
  }

  private _setReadyTimestamp = (): number => {
    const getFutureTimestamp = (offsetSeconds: number): number => {
      const now = Date.now();
  ...
  getMockPrices(db: DB): Record<string, any> {
    const hotels = db.getHotelsByCountryID(this._params.countryID);

    return Object.fromEntries(
      Object.entries(hotels).map(([hotelID]: [string, any]) => {
...
export const searchGeo = (string?: string): Promise<Response> => {
  const addType = (type: string) => (entity: any) => ({ ...entity, type });
